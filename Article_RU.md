...
#### Показатели (Measures)
[Показатели](http://docs.intersystems.com/latest/csp/docbook/DocBook.UI.Page.cls?KEY=D2MODEL_ch_concepts#D2MODEL_concepts_measure) или метрики это такое свойство куба, куда относят какие-либо "количественные" данные, которые необходимо посчитать для "качественных" измерений куба (Dimensions). 
...
Портал Пользователя DeepSee - веб-приложение в котором пользователи используя созданные ранее сводые таблицы визуализируют их. Минимальная единица визуализации - виджет (widget), это может быть таблица, график и т.п. Виджеты объединены в дэшборды. Для перехода к Порталу Пользователя DeepSee откроем Портал Управления Системой → DeepSee → Выбор области →  Портал Пользователя. 
